...
def train(nb_epoch, batch_size, store_name, patience=10, min_delta=0.001, resume=False, start_epoch=0, model_path=None,
          data_path=''):
    exp_dir = store_name
    try:
        os.stat(exp_dir)
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):  
            ...
            optimizer.zero_grad()
            inputs3 = inputs
            output_1, output_2, output_3, a, map1, map2, map3 = netp(inputs3)
            ...
            if batch_idx % 20 == 0:
                print(
                    'Step: %d | Loss1: %.3f | Loss2: %.5f | Loss3: %.5f | Loss_ATT: %.5f |Loss_concat: %.5f | Loss: '
                    '%.3f | Acc: %.3f%% (%d/%d)' % (
        ...
        train_losses1.append(train_loss1 / (idx + 1))
        train_losses2.append(train_loss2 / (idx + 1))
        train_losses3.append(train_loss3 / (idx + 1))
        train_losses4.append(train_loss4 / (idx + 1))
        train_losses5.append(train_loss5 / (idx + 1))

        with open(exp_dir + '/results_train_new.txt', 'a') as file:
            file.write(
                'Iteration %d | train_acc = %.5f | train_loss = %.5f | Loss1: %.3f | Loss2: %.5f | Loss3: %.5f | '
                'Loss_ATT: %.5f | Loss_concat: %.5f |\n' % (
                    epoch, train_acc, train_loss, train_loss1 / (idx + 1), train_loss2 / (idx + 1),
                    train_loss3 / (idx + 1),
                    train_loss4 / (idx + 1), train_loss5 / (idx + 1)))

        if epoch < 5 or epoch >= 5:
            val_acc_com, val_loss = test(net, CELoss, 1, data_path + '/test')
            if val_acc_com > max_val_acc:
                max_val_acc = val_acc_com
                net.cpu()
                torch.save(net, './results/models/model_PIM_CMAL_new.pth')
                net.to(device)
            with open(exp_dir + '/results_test_new.txt', 'a') as file:
                file.write('Iteration %d, test_acc_combined = %.5f, test_loss = %.6f\n' % (
                    epoch, val_acc_com, val_loss))
        #     net.cpu()
        #     torch.save(net, './' + store_name + '/ER_model.pth')
        #     net.to(device)
    ...
    plt.plot(train_losses3, label='Train Loss 3')
    plt.plot(train_losses4, label='Train Loss 4')
    plt.plot(train_losses5, label='Train Loss 5')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
...
if __name__ == '__main__':
    ...
    train(nb_epoch=200,  # number of epoch
          ...
          store_name='./results',  # folder for output
